...
The orchestrator is the only place in the pipeline that touches HeyBap at every level: spec extraction, MCP scaffolding and binding, skill upload, coworker creation, run / log / update, panel rendering, report persistence. You see the whole stack misbehave or fail to expose a needed capability. Surface every finding through [feature-bug-complexity-classification](../feature-bug-complexity-classification/SKILL.md). The router first asks for clarification when one operator answer would resolve a small localized ambiguity; otherwise it classifies SIMPLE vs COMPLEX and dispatches to `bap-bug-report` (opens a PR on `the-agentic-company/bap` and creates a Linear ticket in team `Bap` at status `In Review` linked to the PR), `bap-feature-brainstorm` (creates a Linear ticket in team `Bap` at status `Triage` with label `Need More Shaping` carrying the 3-options problem statement), or `bap-direction-shaping` (opens the fuzzy direction discussion). Linear's own integrations notify the team; no direct Slack post for ticketed findings.
